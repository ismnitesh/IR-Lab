...
void bool_list(map<string, vector<char*> > posting_list, string word1, string word2, int flag)
{
	double ret=0,rel=4,rel_ret=0;
	if(flag==1)
	{
		cout<<word1<<" AND "<<word2<<" :";
		for(int i=0;i<posting_list[word1].size();i++)
		{
			for(int j=0;j<posting_list[word2].size();j++)
			{
				if(strcmp(posting_list[word1][i], posting_list[word2][j])==0)
				{
					ret++;
					cout<<posting_list[word1][i]<<" ";
					if(strcmp(posting_list[word1][i],"4")<=0)
					{
						rel_ret++;
					}
					break;
	...
	else
	{
		...
		for(int i=0;i<posting_list[word1].size();i++)
		{
			or_set.insert(posting_list[word1][i]);
		}
		for(int j=0;j<posting_list[word2].size();j++)
		{
			or_set.insert(posting_list[word1][j]);
		}
		set <char *>::iterator it = or_set.begin();
	...
	double precision = (rel_ret/ret);
	cout << endl;
	cout << "precision: " << precision << endl;
	double recall = (rel_ret/rel);
	cout << "recall: " << recall << endl;
	double f_measure = (2*precision*recall)/(precision+recall);
	cout << "F-measeure: " << f_measure << endl;
}
int main()
{
	...
	cin>>word1>>word2;
	cout<<"Enter 1 for AND & 2 for OR : ";
	cin>>flag;
	bool_list(posting_list, word1, word2, flag);
	return 1;
}
/*
void print_list(map<string, vector<char*> > posting_list)
